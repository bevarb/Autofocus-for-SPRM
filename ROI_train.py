...
parser.add_argument("--dataset_name", type=str, default="New-ROI-Train(move3)_test_loss", help="name of the dataset")
...
parser.add_argument("--lr_D", type=float, default=0.000015, help="adam: learning rate")
...
parser.add_argument("--img_size", type=int, default=256, help="size of image height")
...
train, test = split_TrainTest("./data/New-ROI-Train/for_train/data.csv", "-15", "44")
trainset = TrainDataset(train, "./data/New-ROI-Train/for_train", opt.img_size, opt.channels, flag="not abs")
...
testset = TrainDataset(test, "./data/New-ROI-Train/for_train", opt.img_size, opt.channels, flag="not abs")
...
MSE_loss_min = 100
Loss = [[], []]
Train_Loss = []  # save epoch, batch, gan_loss, l1_loss, l2_loss
Test_Loss = []  # save epoch, batch, gan_loss, l1_loss, l2_loss
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(trainloader):
        ...
        if i % 20 == 0:
            ...
            Train_Loss.append([epoch, i, loss_D.item(), loss_G.item(), loss_GAN.item(), loss_identity.item(), loss_MSE.item()])
    ...
    for i, batch in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            # Total loss
            loss_G = 5 * loss_GAN + 10 * loss_identity + 20 * loss_MSE
            ...
            if i % 20 == 0:
                print(
                    "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, GAN: %f, identity: %f, MSE:%f]"
                    % (
                        epoch,
                        opt.n_epochs,
                        i,
                ...
                Test_Loss.append(
                    [epoch, i, loss_D.item(), loss_G.item(), loss_GAN.item(), loss_identity.item(), loss_MSE.item()])
        if i == 0:
            sample_images(epoch, "test")
    from libs.visulize_results import *
    plot_result(Train_Loss, opt, "Train")
    plot_result(Test_Loss, opt, "Test")

    Train_Loss = pd.DataFrame(Train_Loss, columns=["Epoch", "Batch", "D_Loss", "G_Loss", "Gan_Loss", "L1_Loss", "L2_Loss"])
    Train_Loss.to_csv("./images/Results/New-ROI-Train(move3)_test_loss/Train_Loss_ROI.csv", index=False)
    Test_Loss = pd.DataFrame(Test_Loss, columns=["Epoch", "Batch", "D_Loss", "G_Loss", "Gan_Loss", "L1_Loss", "L2_Loss"])
